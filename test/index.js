...
// var/let/const Global/Local Mutable/Immutable
const collapse = () => {
  console.log("clicked"); //for test
  const navbarList = document.querySelector("#list");
  console.log(navbarList); //for test the variable
  navbarList.classList.toggle("collapsible"); //classList.add(), remove(), replace(), and toggle()
};

toggler.addEventListener("click", collapse); //addEventListener(type, listener)
//get user interact - event handler
...
// FIXME: 3line above/6line below are for previous slider
// const slider = () => {
...
window.onscroll = () => {
  if (
    document.body.scrollTop > 200 || //body element
    document.documentElement.scrollTop > 200 //html element
  ) {
    scrollToTopBtn.style.display = "block"; //Chrome and Safari define their scroll on the <body> element whilst Firefox defines it on the <html>
  } else {
    scrollToTopBtn.style.display = "none"; //scrollTop: property gets or sets the number of pixels that an element's content is scrolled vertically.
  }
};
...
function scrollToTop() {
  ...
  if (c > 0) {
    window.requestAnimationFrame(scrollToTop); //animation for smoth scroll
